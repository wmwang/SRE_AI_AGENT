...
export class LLMLogger {
    ...
    private constructor() {
        ...
        if (process.env.LLM_LOG_PATH) {
            ...
            try {
                ...
                // .../packages/mcp-k8s-deployment/src/utils/llm-logger.ts -> 往上 4 層
        ...
        if (this.enabled) {
            ...
            console.error(`[MCP K8s] LLM Logger enabled: ${this.logPath}`);
        }
    }
     ...
     * 記錄 LLM 請求/回應
     */
    log(type: 'k8s-analysis' | 'k8s-improvement' | 'k8s-repo' | 'other', data: {
        prompt?: string;
        response?: string;
        error?: string;
        ...
        const formatted = `
[${timestamp}] [MCP-K8s: ${type.toUpperCase()}]
${'-'.repeat(80)}
